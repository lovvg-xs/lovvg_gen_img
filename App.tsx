...
const App: React.FC = () => {
  ...
  const [imageStyle, setImageStyle] = useState<ImageStyle>(ImageStyle.REAL);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    let totalGenerations = promptsList.length * imagesPerPrompt;
    ...
    for (let i = 0; i < promptsList.length; i++) {
      ...
      for (let j = 0; j < imagesPerPrompt; j++) {
        currentGeneration++;
        const apiKey = settings.apiKeys[(i * imagesPerPrompt + j) % settings.apiKeys.length];
        
        setLoadingMessage(`Prompt ${i + 1}/${promptsList.length}, Img ${j + 1}/${imagesPerPrompt}`);
        ...
        try {
          ...
          setGeneratedImages(prev => [
            { id: `${Date.now()}-${i}-${j}`, url: imageUrl, prompt: prompt },
            ...prev, 
          ]);
        ...
        // Delay between calls to respect API rate limits, but not after the very last one
    ...
    setIsGenerating(false);
    setLoadingMessage('');
  }, [prompts, imageStyle, aspectRatio, settings.apiKeys, setGeneratedImages, showToast, imagesPerPrompt]);
  ...
  const handleNextPreview = useCallback(() => {
    ...
    const currentIndex = generatedImages.findIndex(img => img.id === previewImage.id);
    if (currentIndex > -1) {
        const nextIndex = (currentIndex - 1 + generatedImages.length) % generatedImages.length;
  ...
  const handlePrevPreview = useCallback(() => {
      ...
      const currentIndex = generatedImages.findIndex(img => img.id === previewImage.id);
      if (currentIndex > -1) {
          const prevIndex = (currentIndex + 1) % generatedImages.length;
  ...
  }, [previewImage, handleNextPreview, handlePrevPreview]);
    ...
    <div className="min-h-screen bg-gray-900 text-white p-4 md:p-8 font-press-start">
             ...
             {generatedImages.length > 1 && (
              <>
                <button 
                  ...
                  className="absolute left-0 sm:-left-16 top-1/2 -translate-y-1/2 z-20 bg-gray-800/70 text-white p-2 rounded-full border-2 border-white/50 hover:bg-gray-700 hover:border-white transition-all" aria-label="Previous Image">
                  <ChevronLeftIcon className="w-8 h-8"/>
                </button>
                 <button 
                  onClick={handleNextPreview} 
                  className="absolute right-0 sm:-right-16 top-1/2 -translate-y-1/2 z-20 bg-gray-800/70 text-white p-2 rounded-full border-2 border-white/50 hover:bg-gray-700 hover:border-white transition-all" aria-label="Next Image">
                  <ChevronRightIcon className="w-8 h-8"/>
                </button>
              </>
            )}
            <div className="flex flex-col items-center gap-4">
              <img src={previewImage.url} alt={previewImage.prompt} className="max-w-full max-h-[75vh] object-contain pixelated" />
              <p className="text-center text-gray-300 text-sm mt-2">{previewImage.prompt}</p>
            </div>
          </div>
        )}
      </Modal>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 bg-gray-800 p-6 border-4 border-black shadow-[8px_8px_0_0_#000] flex flex-col gap-6 self-start">
          ...
          <div>
            <label htmlFor="style-select" className="block mb-2 text-lg text-gray-300">Style</label>
            <div className="relative">
                <select
                id="style-select"
                value={imageStyle}
                ...
                disabled={isGenerating}
                className="w-full p-4 bg-gray-900 border-2 border-gray-600 focus:border-cyan-400 focus:outline-none appearance-none cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed selection:bg-cyan-400 selection:text-black"
              >
                <option value={ImageStyle.PIXEL}>Pixel Art</option>
                <option value={ImageStyle.STICK_FIGURE}>Stick Figure</option>

                <option value={ImageStyle.CARTOON}>CARTOON</option>
                <option value={ImageStyle.REAL}>REAL</option>
              </select>
          ...
          <div>
            <label htmlFor="image-count" className="block mb-2 text-lg text-gray-300">Images Per Prompt</label>
            <input
              type="number"
              id="image-count"
              ...
              onChange={(e) => {
                ...
                if (val > 16) {
                    setImagesPerPrompt(16);
                    showToast("Max images per prompt is 16.", "info");
                } else if (val < 1) {
                    setImagesPerPrompt(1);
                } else {
                    setImagesPerPrompt(val);
                }
              }}
              min="1"
              max="16"
              disabled={isGenerating}
              className="w-full p-3 bg-gray-900 border-2 border-gray-600 focus:border-cyan-400 focus:outline-none selection:bg-cyan-400 selection:text-black [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
            />
            <p className="mt-2 text-xs text-gray-500">Number of images for each prompt (1-16).</p>
          </div>
            ...
            <div className="grid grid-cols-3 gap-2">
                {(["1:1", "16:9", "9:16", "4:3", "3:4"] as const).map((ratio) => (
                    <button
                        ...
                        disabled={isGenerating}
                        className={`p-3 text-sm border-2 transition-colors disabled:opacity-50 hover:border-cyan-400 ${
                          aspectRatio === ratio
